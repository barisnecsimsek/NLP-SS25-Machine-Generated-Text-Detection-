...
model's layers, LoRA drastically reduces the number of trainable parameters \cite{hu2022lora}. We trained RoBERTa-base for 3 epochs with a learning rate of 2e-4 
and a batch size of 16. XLM-R was trained for 4 epochs with a learning rate of 2e-5. For the LoRa configurations, we set the rank of the low-rank matrices to 64, 
...
First of all, we see that the best accuracy of 77.3\% is achieved by only adding readability features to the RoBERTa baseline. With a slight 
...
RoBERTa embeddings with lexical diversity features including type-token ratio and hypergeometric distribution \emph{d} (HDD). When employing this feature
