...
a critical task is to distinguish between human written and machine-generated text. This paper describes our work as 
part of the NLP Bachelor Praktikum SS2025 on the SemEval 2024 Task 8: \emph{Multidomain, Multimodel and Multilingual 
Machine-Generated Text Detection}. Our method focused on combining RoBERTa embeddings with linguistic and 
token probabilistic features on tackling Subtask A: Human vs Machine Text Classification. The final model’s architecture
was systematically developed on the monolingual track to obtain a reliably working variant, which later was adapted and 
evaluated in a multilingual context. 
Our results demonstrate that this hybrid approach can compete with the top performing teams of the competition, 
achieving an accuracy of 88.64\% on the official test dataset for the monolingual, and an accuracy of x\% for the 
multilingual track. 
...
\textbf{Flesch Reading Ease and Flesch-Kincaid Grade Level.} Focuses on sentence length and syllable count per word.
{\small
\[
    \text{FK} = 0.39 \times \frac{\text{total words}}{\text{total sentences}} + 11.8 \times \frac{\text{total syllables}}{\text{total words}} - 15.59
\]}

\textbf{Gunning Fog Index.} Asses sentence complexity by analyzing polysyllabic words.
{\small
\[
    \text{GFI} = 0.4 \times \left(\frac{\text{total words}}{\text{total sentences}} + 100 \times \frac{\text{complex words}}{\text{total words}}\right)
\]}

\textbf{Automated Readability Index.} Counts number of characters per word and number of words per sentence. 
{\small
\[
    \text{ARI} = 4.71 \times \frac{\text{characters}}{\text{words}} + 0.5 \times \frac{\text{words}}{\text{sentences}} - 21.43
\]}
...
hypotheses of machine-generated text being more predictable than human-written text, as humans tend to write in a more 
surprising way, while LLMs are optimized to choose the next most likely token \cite{holtzman2019curious}. 
...
\(i\)-th position we can extract the probability distribution \(P(x_{i+1} \mid x_1, \dots, x_i)\) over the entire GPT-2 
...
step. 
Finally, we take (iii) the entropy, which measures the uncertainty of the distribution. A low entropy would indicate that 
...
Table \ref{tab:Dataset} depicts the dataset statistics. Machine-generated texts were generated by prompting to write an article 
given a title (Wikipedia) or an abstract given a paper title (arXiv) and similar prompting for other domains. 
